...
class Table_InventoryBatchLog
{
	...
	public function getJoinProductByCondition($condition = array(), $type = '*', $pageSize = 0, $page = 1, $orderBy = "")
	{
        ...
        $table = $this->_table->info('name');
        $select->from($table, $type);
		$select->join('product', 'product.product_id='.$table.'.product_id',array('product_title','product_title_en','product_sku'));
		$select->where("1 =?", 1);
		
		if(isset($condition["customer_code"]) && $condition["customer_code"] != ""){
            $select->where("product.customer_code = ?",$condition["customer_code"]);
        }
		if(isset($condition["time_start"]) && $condition["time_start"] != "" && isset($condition["time_end"]) && $condition["time_end"] != ""){
            $select->where("ibl_add_time >= ?",$condition["time_start"]);
			$select->where("ibl_add_time < ?",$condition["time_end"]);
        }
        if (!empty($orderBy)) {
            $select->order($orderBy);
        }
