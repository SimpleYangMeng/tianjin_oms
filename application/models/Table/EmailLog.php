<?php
class Table_EmailLog
{
    protected $_table = null;

    public function __construct()
    {
        $this->_table = new DbTable_EmailLog();
    }
    ...
    public function getByCondition($condition = array(), $field = '*', $order = '', $pageSize = 20, $page = 0)
    {
    	...
    	if (isset($condition["email_status"]) && $condition["email_status"] !== "") {
    		$select->where("{$table}.email_status = ?", $condition["email_status"]);
    	}
    	if (isset($condition["email_type"]) && $condition["email_type"] != "") {
    		$select->where("{$table}.email_type = ?",$condition["email_type"]);
    	}

    	if (isset($condition["php_session_code"]) && $condition["php_session_code"] != "") {
    		$select->where("{$table}.php_session_code = ?",$condition["php_session_code"]);
    	}		
