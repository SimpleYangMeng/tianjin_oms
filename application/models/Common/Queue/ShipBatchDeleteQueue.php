...
class ShipBatchDeleteQueue
{
    ...
    {
        $shipBatchRows = Service_ShipBatch::getByMarkDeleteStatus( $this->status['send_before'] , $pageSize , $minId);
        ...
        foreach ($shipBatchRows as $key => $val) {
            ...
            $updateFlag = Service_ShipBatch::update(array(
                'mark_delete' => $this->status['send_after']
            ) , $val['sb_id']);
            //修改失败，这条先不插入队列
            if($updateFlag === false){
               continue;
    ...
    private function pageInfo()
    {
        $rowTotal = Service_ShipBatch::getByCondition(array(
            'mark_delete' => $this->status['send_before'],
        ) , 'count(*)');
