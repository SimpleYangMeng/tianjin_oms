...
*
*/
class RecordCompaniesSendMessage extends SendMessageParent
{
    ...
    public function createMessage($itemRow)
    {
        ...
        //$validityDate = Common_Common::getEndDate($customerData['validity_date']);
        ...
        $declaration = array(
            'ENTERPRISE' => array(
                ...
                'FORM_TYPE' => $businessType?$businessType:'',//业务类型(保税进口:BI，保税出口:BE，一般进口:NI，一般出口:NE) 目前最多支持5位
                'ATTACHMENT' => array(//附件
                        'NAME'=>'',
                        'CONTENT' => '',
                )
     ...
     * @return [type]                 [description]
     */
    protected function handleReceipt($receiveReceipt)
    {
        // 失败
        if($sendStatus['ask'] == 0){
