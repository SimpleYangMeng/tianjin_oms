	...
	protected function check($isInterface = false) {
		...
		*/
		...
		if (empty($this->_param['id'])) {
			$tmpRow = Service_Waybill::getByCondition(array('logistic_customer_code'=>$this->_addData['logistic_customer_code'], 'reference_no' => $this->_param['referenceNo']), array('wb_code'));
			if ($tmpRow) {
				$this->_error[] = '交易单号(' . $this->_param['referenceNo'] . ')已存在, 运单编号：'.$tmpRow[0]['wb_code'];
				return;
			}

			$tmpRow = Service_Waybill::getByCondition(array('logistic_customer_code'=>$this->_addData['logistic_customer_code'], 'log_no' => $this->_param['logNo']), array('wb_code'));
			if ($tmpRow) {
				$this->_error[] = '物流运单编号(' . $this->_param['logNo'] . ')已存在, 运单编号：'.$tmpRow[0]['wb_code'];
				return;
			}
		}
...
*/
        ...
        if($this->_param['shipName'] != '-'){
			...
			if (!$trafTools) {
				$this->_error[] = '运输工具(' . $this->_param['shipName'] . ')不存在';
			}
        }
		//检查订单表里的交易单号是否存在
