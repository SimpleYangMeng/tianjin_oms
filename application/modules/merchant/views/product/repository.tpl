<div class="content-box  ui-tabs  ui-corner-all">
	<form action="/merchant/product/repository" method="post" id="pagerForm">
        ...
        <input type="hidden" name="cusCode" value="<{$cusCode}>" id="cusCode" />
		<div class="searchBar">
		...
		<tbody>
			<{if $result|@count gt 0}>
				<{foreach from=$result item=item}>
					<tr>
						<td align="center" style="width:15px; text-align:center"><input type="checkbox" name="pid"  productSku="<{$item['backup_code']}>" productId="<{$item['product_id']}>" productName="<{$item['product_title']}>" productSalesValue="<{$item['product_sales_value']}>" category="<{$item['pc_name']}>" productWeight="<{$item['product_weight']}>" option="{id:<{$item['product_id']}>,sku:'<{$item['backup_code']}>'}"  class="<{$from}>actionSku" /></td>
